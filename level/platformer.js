...
function getHistoryCategoryInfo(item) {
  const type = String(item?.type || '').toLowerCase();
  const detail = String(item?.detail || '');

  if (detail.includes('상승')) {
    return { cardClass: 'history-card-rise', badgeClass: 'badge-type-rise', label: '순위 상승' };
  }
  if (detail.includes('하락')) {
    return { cardClass: 'history-card-drop', badgeClass: 'badge-type-drop', label: '순위 하락' };
  }
  if (detail.includes('재평가') || detail.includes('조정') || detail.includes('재조정')) {
    return { cardClass: 'history-card-re-eval', badgeClass: 'badge-type-re-eval', label: '순위 재평가' };
  }
  if (type === 'add' || detail.includes('등록') || detail.includes('새로운 레벨')) {
    return { cardClass: 'history-card-add', badgeClass: 'badge-type-add', label: '신규 등록' };
  }
  if (type === 'remove' || detail.includes('삭제')) {
    return { cardClass: 'history-card-remove', badgeClass: 'badge-type-remove', label: '레벨 삭제' };
  }
  if (type === 'move') {
    return { cardClass: 'history-card-re-eval', badgeClass: 'badge-type-re-eval', label: '순위 변동' };
  }
  return { cardClass: 'history-card-default', badgeClass: 'badge-type-default', label: '기록' };
}
  ...
  detailContainer.innerHTML = `
    ...
    ${(() => {
      ...
      return `
        ...
        const info = getHistoryCategoryInfo(item);
        return `
                <li class="detail-history-item ${info.cardClass}">
                  <span class="detail-history-badge ${info.badgeClass}">${info.label}</span>
...
async function initPlatformerPage() {
  ...
  if (searchInput) {
    const handlePlatformerSearch = window.debounce ? window.debounce(() => {
      renderLevelList(cachedLevelsData);
  ...
  window.addEventListener('forumNicknameChanged', refreshLevelView);
  window.addEventListener('storage', (e) => {
    if (e.key === 'forumNickname') refreshLevelView();
  });

  const loadPlatformerData = async () => {
    try {
      const response = await fetch('platformer.json');
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();

      const merged = applyDevCustomData(data.levels ?? [], data.history ?? [], 'platformer');
      ...
      console.error('platformer.json load failed:', err);
    }
  };
