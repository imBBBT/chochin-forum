...
async function initChallengePage() {
  ...
  if (searchInput) {
    const handleChallengeSearch = window.debounce ? window.debounce(() => {
      renderLevelList(cachedLevelsData);
  ...
  const loadChallengeData = async () => {
    try {
      ...
      const merged = applyDevCustomData(data.levels ?? [], data.history ?? [], 'challenge');
      ...
      console.error('challenge.json load failed:', err);
    }
  };

  window.addEventListener('devDataUpdated', loadChallengeData);
